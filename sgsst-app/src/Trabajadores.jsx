...
import { Users, UserPlus, HeartPulse, Hospital, CalendarClock, Eye, Search, PlusCircle, AlertCircle, Phone, Save, X } from 'lucide-react';
...
export default function Trabajadores({ companyProfile }) {
  ...
  const saveTrabajador = async () => {
    ...
    if (!companyProfile?.id) {
      alert("Error: No hay una empresa seleccionada. Por favor reinicie sesión.");
      return;
    }

    try {
      const response = await api('/empresas/trabajadores', {
        method: 'POST',
  ...
  const saveAusencia = async () => {
    ...
    try {
      const response = await api(`/empresas/trabajadores/${ausenciaForm.trabajador_id}/ausentismos`, {
        method: 'POST',
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl">
             <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
               <h3 className="text-lg font-bold text-gray-800 flex items-center"><UserPlus className="w-5 h-5 mr-2 text-orange-500"/> Registrar Trabajador</h3>
               <button onClick={() => setShowTrabajadorModal(false)} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5"/></button>
             </div>
             <div className="p-6 grid grid-cols-2 gap-4">
                <div className="col-span-2 md:col-span-1">
                   <label className="block text-xs font-bold text-gray-700 mb-1">Nombre Completo *</label>
                   <input type="text" value={trabajadorForm.nombre_completo} onChange={e => setTrabajadorForm({...trabajadorForm, nombre_completo: e.target.value})} className="w-full rounded-lg border-gray-200 text-sm focus:ring-orange-500 focus:border-orange-500" />
                </div>
                <div className="col-span-2 md:col-span-1">
                   <label className="block text-xs font-bold text-gray-700 mb-1">Cédula</label>
                   <input type="text" value={trabajadorForm.documento} onChange={e => setTrabajadorForm({...trabajadorForm, documento: e.target.value})} className="w-full rounded-lg border-gray-200 text-sm focus:ring-orange-500 focus:border-orange-500" />
                </div>
                <div className="col-span-2 md:col-span-1">
                   <label className="block text-xs font-bold text-gray-700 mb-1">Cargo / Puesto</label>
                   <input type="text" value={trabajadorForm.cargo} onChange={e => setTrabajadorForm({...trabajadorForm, cargo: e.target.value})} className="w-full rounded-lg border-gray-200 text-sm focus:ring-orange-500 focus:border-orange-500" />
                </div>
                <div className="col-span-2 md:col-span-1">
                   <label className="block text-xs font-bold text-gray-700 mb-1">Fecha de Ingreso</label>
                   <input type="date" value={trabajadorForm.fecha_ingreso} onChange={e => setTrabajadorForm({...trabajadorForm, fecha_ingreso: e.target.value})} className="w-full rounded-lg border-gray-200 text-sm focus:ring-orange-500 focus:border-orange-500" />
                </div>
                ...
                <div className="col-span-2 md:col-span-1">
                   ...
                   <select value={trabajadorForm.tipo_sangre} onChange={e => setTrabajadorForm({...trabajadorForm, tipo_sangre: e.target.value})} className="w-full rounded-lg border-gray-200 text-sm focus:ring-rose-500 focus:border-rose-500">
                ...
                <div className="col-span-2 md:col-span-1">
                   ...
                   <input type="text" value={trabajadorForm.contacto_emergencia_nombre} onChange={e => setTrabajadorForm({...trabajadorForm, contacto_emergencia_nombre: e.target.value})} className="w-full rounded-lg border-gray-200 text-sm focus:ring-rose-500 focus:border-rose-500" />
                </div>
                <div className="col-span-2 md:col-span-1">
                   <label className="block text-xs font-bold text-gray-700 mb-1">Teléfono Emergencia *</label>
                   <input type="text" value={trabajadorForm.contacto_emergencia_telefono} onChange={e => setTrabajadorForm({...trabajadorForm, contacto_emergencia_telefono: e.target.value})} className="w-full rounded-lg border-gray-200 text-sm focus:ring-rose-500 focus:border-rose-500" />
                </div>
             </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl">
             ...
             <div className="p-6 pt-0 space-y-4">
                <div>
                   ...
                   <select value={ausenciaForm.trabajador_id} onChange={e => setAusenciaForm({...ausenciaForm, trabajador_id: e.target.value})} className="w-full rounded-lg border-gray-200 text-sm focus:ring-rose-500 focus:border-rose-500">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                       ...
                       <input type="date" value={ausenciaForm.fecha_inicio} onChange={e => setAusenciaForm({...ausenciaForm, fecha_inicio: e.target.value})} className="w-full rounded-lg border-gray-200 text-sm focus:ring-rose-500 focus:border-rose-500" />
                    </div>
                    <div>
                       <label className="block text-xs font-bold text-gray-700 mb-1">Fecha Retorno (Fin)</label>
                       <input type="date" value={ausenciaForm.fecha_fin} onChange={e => setAusenciaForm({...ausenciaForm, fecha_fin: e.target.value})} className="w-full rounded-lg border-gray-200 text-sm focus:ring-rose-500 focus:border-rose-500" />
                    </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-xs font-bold text-gray-700 mb-1">Días Incapacidad *</label>
                        <input type="number" min="1" value={ausenciaForm.dias_incapacidad} onChange={e => setAusenciaForm({...ausenciaForm, dias_incapacidad: e.target.value})} className="w-full rounded-lg border-gray-200 text-sm focus:ring-rose-500 focus:border-rose-500" />
                    </div>
                    <div>
                       <label className="block text-xs font-bold text-gray-700 mb-1">Causa Exigida *</label>
                       <select value={ausenciaForm.causa} onChange={e => setAusenciaForm({...ausenciaForm, causa: e.target.value})} className="w-full rounded-lg border-gray-200 text-sm focus:ring-rose-500 focus:border-rose-500 bg-gray-50">
                ...
                <div>
                   ...
                   <textarea rows="2" value={ausenciaForm.observaciones} onChange={e => setAusenciaForm({...ausenciaForm, observaciones: e.target.value})} className="w-full rounded-lg border-gray-200 text-sm focus:ring-rose-500 focus:border-rose-500" placeholder="Notas adicionales..."></textarea>
                </div>
             </div>
             <div className="p-4 bg-gray-50 flex justify-end gap-3 border-t border-gray-100">
